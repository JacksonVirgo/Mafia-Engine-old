...
const noArgs = (message, _args) => {
    let mainCommands = [];
    let alias = {};
    for (const commandHandle of Command_1.commandHandles) {
        let { command } = commandHandle;
        let fetchAlias = Command_1.commandAlias[command];
        if (!fetchAlias)
    ...
    for (const cmd of mainCommands) {
        embedDesc += `\n**${cmd}** - ${Command_1.commandDescriptions[cmd]}`;
    }
...
const withArgs = (message, args) => {
    let command = Command_1.messageCommands[args[0]];
    if (!command)
        command = Command_1.messageCommands[Command_1.commandAlias[args[0]]];
    if (!command)
        return message.channel.send(`Command does not exist. Did you mean ${'`'}${(0, Command_1.getClosestCommand)(args[0])}${'`'}`);
    let perms = '';
    let permAppend = '';
