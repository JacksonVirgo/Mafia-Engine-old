...
class VoteCountForm extends ToolRoot {
    ...
    render() {
        return (
            <CenterModal title='Vote Counter' child={(
                <form className={style.modalForm} onSubmit={this.onFormSubmit.bind(this)}>
                    <label className={style.modalLabel} htmlFor='gameUrl'>Link to Game Thread</label>
                    ...
                    <textarea className={style.modalTextarea} name='result' value={this.state.result} readOnly />
                </form>
            )} />
        );
    }
    ...
    format(voteData) {
        ...
        for (const category in wagons) {
            ...
            for (const wagonHead in wagons[category]) {
                ...
                totalVoteArray.push({ size: voteArray.length, wagon: `${vote} [b][E-${majority - voteArray.length}][/b]\n` });
            }
